package com.shop.backend.generator;
...
public class MyBatisPlusGenerator {
    ...
    /** 数据库连接URL */
    private static final String DB_URL = "jdbc:mysql://localhost:3306/shixunguanli?serverTimezone=Asia/Shanghai&useUnicode=true&characterEncoding=utf-8&useSSL=false";
    /** 数据库用户名 */
    private static final String DB_USERNAME = "root";
    /** 数据库密码 */
    private static final String DB_PASSWORD = "111666";

    // ============== 生成器配置 ==============
    /** 生成代码的作者（会显示在类注释中） */
    private static final String AUTHOR = "MisterDong";
    /** 父包名（生成的代码会放在此包下的 entity/mapper/service/controller 子包中） */
    private static final String PARENT_PACKAGE = "com.shop.backend";
    ...
    public static void main(String[] args) {
        ...
        String projectPath = resolveBackendModulePath();
    ...
    private static String resolveBackendModulePath() {
        Path userDir = Path.of(System.getProperty("user.dir")).toAbsolutePath().normalize();
        if (Files.exists(userDir.resolve("pom.xml")) && Files.isDirectory(userDir.resolve("src/main/java"))) {
            return userDir.toString();
        }
        Path backendModule = userDir.resolve("springboot/backend").normalize();
        if (Files.exists(backendModule.resolve("pom.xml"))) {
            return backendModule.toString();
        }
        return userDir.toString();
    }
}
